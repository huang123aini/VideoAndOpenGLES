...
#import "HVideoDecoder.h"
#import "GPUImageContext.h"
#import "HColorConversion.h"


@interface HImageView () <HVideoDecoderDelegate>
{
...
@property (nonatomic, strong) HVideoDecoder *decoder;
...
- (instancetype)initWithFrame:(CGRect)frame videoPath:(NSString *)videoPath previewImage:(UIImage *)previewImage
{
    ...
    if (self)
    {
        ...
        _decoder = [[HVideoDecoder alloc] initWithVideoPath:videoPath size:frame.size];
...
- (void)didDecodeInputFramebuffer:(GPUImageFramebuffer *)newInputFramebuffer inputSize:(CGSize)newSize frameTime:(CMTime)frameTime
{
    ...
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        glVertexAttribPointer(displayTextureCoordinateAttribute, 2, GL_FLOAT, 0, 0, [HImageView textureCoordinatesForRotation:self.rotationMode]);
